...
// Why: Parent components (like LandingPage) re-render during modal interactions, booking dialogs, or auth state changes.
// Impact: Reduces main thread blocking during renders, skipping reconciliation of all internal elements and improving UI responsiveness.
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3.5 items-end">
          ...
          <div className="form-group md:col-span-3 flex flex-col gap-1.5 mb-0">
            ...
            <div className="relative">
              <select
                ...
                className="w-full h-11 rounded-xl border border-slate-200 dark:border-slate-700 px-3.5 text-sm bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-100 font-medium focus:bg-white dark:focus:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 transition form-select"
                value={district}
          ...
          <div className="md:col-span-3 flex flex-col gap-2 w-full min-w-[160px]">
            <button
              ...
              className="btn btn-primary h-11 w-full flex items-center justify-center gap-1.5 bg-emerald-800 hover:bg-emerald-900 disabled:opacity-70 text-white font-semibold px-4 rounded-xl shadow-sm hover:shadow transition text-sm whitespace-nowrap"
            >
              {isSearching ? (
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all duration-150 cursor-pointer flex items-center gap-1.5 facility-pill min-h-[32px] select-none ${active
                  ? 'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-800 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800 font-semibold shadow-xs ring-1 ring-emerald-500/20'
                  : 'bg-slate-50 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 border-slate-200/70 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 hover:border-emerald-300'
                  }`}
              >
